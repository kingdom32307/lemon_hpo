...
class BaseOptimizer():
    ...
    def optimize(self):
        ...
        hps_conf, losses, best_id = self.hp_utils.load_hps_conf(do_sort=True)
        # best_model_path = "history/stdo" + self.hp_utils.save_path[11:] + "/log{:0>5}.pth".format(best_id)
        # shutil.copy2(best_model_path, self.hp_utils.save_path+ "/best_model.pth")
    ...
    def _optimize_sequential(self, save_time):
        while True:
            ...
            hps_conf, losses, best_id = self.hp_utils.load_hps_conf(do_sort=True)
            best_model_path = "history/stdo" + self.hp_utils.save_path[11:] + "/log{:0>5}.pth".format(best_id)
            print("Best Model is log{:0>5}.pth".format(best_id))
            shutil.copy2(best_model_path, self.hp_utils.save_path+ "/best_model.pth")
